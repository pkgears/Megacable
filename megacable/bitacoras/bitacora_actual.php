...
<body>
	<div class="" id="contenido">
		...
		<section id="bitacora_div" >
			<?php 
				include ('../php/conexion.php');
				$link=conectar();
				$id=$_GET['id'];
				$sql_select="SELECT * from index_bitacora where id_bit=(SELECT MAX(id_bit) from index_bitacora where id_tecnico='$id');";
				...
				$sql_tecnico="SELECT id_tecnico,nombre,apellido from tecnicos where id_tecnico='$id';";
				$result_tecnico=mysql_query($sql_tecnico,$link);
				$row_tecnico=mysql_fetch_array($result_tecnico);

				echo "<h3>Bitacora de ".$row_tecnico['nombre']." ".$row_tecnico['apellido']."</h3>";
				$sql_bit="SELECT * FROM bitacoras where id_bit='$id_bitacora' order by captura desc;";
				$result_bitacora1=mysql_query($sql_bit,$link);
				$result_bitacora2=mysql_query($sql_bit,$link);
				if($row_bit=mysql_fetch_array($result_bitacora1)){
					echo "<table border='1px' class='tabla_bitacora'>
							<form action='guardar_link.php' method='POST'>			
							<tr>
								<th>Fecha</th>
								<th>No. serie</th>
								<th>Suscriptor</th>
								...
								<th>Conector Rg-6</th>
								<th>Grapa tipo O</th>
								<th>Tapon Sella-muros</th>
								<th>Cinta ponchable</th>
								<th>Access</th>
								<th>Modem</th>
							</tr>";
					while($row_bit=mysql_fetch_array($result_bitacora2)){
						echo "	<tr>
								<td>".$row_bit['fecha']."</td>
					 ...
					 echo "<input type='hidden' value=".$tecnico." name='id'>
							<input type='hidden' value=".$id_bitacora." name='id_bitacora'>";
					echo "<td ><input type='date'  name='fecha' size='3'></td>
			...
			<input type='submit' value='Guardar'>
					...
					echo "<h3>***No hay registro de bitácora***</h3>
					";
				}
				 ?>	
			</section>
