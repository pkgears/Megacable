	...
	<div class="" id="contenido">
		...
		<section id="central" class='add_foto'>
				...
				<?php 
					...
					echo "<h3>Material entregado a ".$row_tecnico['nombre']." ".$row_tecnico['apellido']." </h3>";
					$sql_rec="SELECT * FROM material_entregado where id_entrega=(Select MAX(id_entrega) from material_entregado) and id_tecnico='$tecnico' ;";
					...
					echo '</table><a href="mod_inventario_tecnico.php?fecha='.$row1['fecha'].'&tecnico='.$tecnico.'"><img src="edit_inv.png" alt="" title="Modificar el material recibido"></a>
							<br><br>';
						}
						else{
			...
			<?php  
			 ?>
				...
				<?php
					echo "<h3>Material utilizado por ".$row_tecnico['nombre']." ".$row_tecnico['apellido']." </h3>"; 
					$link=conectar();

					$sql_util="SELECT * FROM material_utilizado_tecnicos where id_util=(Select MAX(id_util) from material_utilizado_tecnicos) and id_tecnico='$tecnico' ;";
				...
				<?php 
					echo "<h3>Inventario final de ".$row_tecnico['nombre']." ".$row_tecnico['apellido']." </h3>";
					$link=conectar();
					...
					$sql_inv="SELECT * FROM inventario_tecnico where id_inv_tec=(Select MAX(id_inv_tec) from inventario_tecnico) and id_tecnico='$tecnico';";
					$resultado_inv=mysql_query($sql_inv,$link);

					$sql_inv_update="UPDATE `inventario_tecnico` SET `barril`='$barril', divisor2='$divisor2', divisor3='$divisor3', grapaRG6='$grapaRG6', coaxialRG6='$coaxialRG6', coaxialRG6auto='$coaxialRG6auto', conectorRG6='$conectorRG6', grapaO='$grapaO', tapon_sellamuros='$ts', cinta_ponchable='$cp', access='$access', modem='$modem' WHERE id_tecnico='$tecnico' ;";
